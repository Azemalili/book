...
function Home() {
  ...
  return (
    <div>
        <div>
          <h3 style={{textAlign: 'center', marginTop: '90px', marginBottom: '30px'}}>New books</h3>
          <div id='latestbooks'>
            {show && show.map((item, index)=> {return <Link key={index} to={`/Book/${item.id}`} ><CardBook item={item} index={index} id='card' /></Link>})}
          </div>
        </div>
